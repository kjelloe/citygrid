...
try {
  ...
  const OPENERS = {
    ...
    // Photo mode's slim bar (F1). Its opener is the camera cluster's photo
    // button, which stays on screen IN photo mode — the same rule street mode
    // follows, and this gate is why: an opener that hides itself cannot be a
    // toggle, and a player who cannot find Escape would have no way out they
    // can see. K1 moved it out of the top bar so the camera has one home.
    photobar: "#camera-photo",
  };

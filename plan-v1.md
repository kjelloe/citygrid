...
|---|---|---|---|---|
| **V1** | **Traffic you can see** — moving cars driven by `state.tiles.traffic` and the road connection mask | M | The engine has computed a per-tile commuter load since N7, it is hashed state, and the only thing that reads it is an overlay tint and one inspector row. The single largest gap to the reference, and the one that needs no new rules | **Done (slice V1).** `client/life/traffic.js`: cars on the lane graph, density AND speed from `tiles.traffic` — a full byte of load runs at 35% of the limit and settles at 8 cars per 100 m, an empty road at 9.6 m/s and 1.9. Renderer-side, no hash moved. 997 cars on a 64×64 at 0.09 ms a step, 3,660 on a 128×128 at 0.5 ms. A car is 82 measured triangles, so it is a ladder rung between props and markings and a resolvability gate at 18 px. `?life=0` freezes them and two screenshots of one city are byte identical |

...
| **A style is not a filter** (§3) | A visual style owns geometry, shading and palette; the post-process is the least of the four and two of the three candidates use none at all. A screen-space outline actively fights detailed geometry — the more windows and roof clutter a building gains, the more the edge test fires, until the image is mud (ruling 017). |
| **Buildings are individual** (§3) | Four silhouettes per category, chosen from the building's id, with per-building height, colour and quarter-turn rotation on top. None of it is game state: shape, spin, colour, tree species and car colour all derive from ids, so two clients agree without any of it being saved, replayed or hashed. |
...
| Level of detail | A configurable triangle budget (`setBudget`, default 80,000) spent by two gates: what is resolvable at the current zoom, then a fixed ladder of sacrifices. Enforced against the renderer's own triangle counter after an actual render rather than against a cost model, because the model was wrong four times running (ruling 019). Measured 19k–69k across every zoom on a saturated 128×128 sixteen-seat region. Terrain sets a floor that no amount of stripping goes below, which is why the default is 80k and not 40k — roads were part of that floor until N30 painted them into the terrain mesh. **The cost model was measured for buildings and trees and remembered for the ground, and went 23% wrong the moment a road became a skirted box (P35).** It is measured throughout now, and `tools/budget_gate.mjs` compares the estimate with what three actually drew |
...
| Traffic | One multi-source BFS distance field from every job per month; each home walks downhill through it laying load. 0.70ms on a saturated 128×128 against an 8ms budget. Congestion correlates with people-per-road (r=0.55) and not with the seed (r=-0.08) — in era 1; since B9 (era 3) the deputy's road grows with its town, people-per-road stopped standing in for load (r=0.07), and the gate reads driving demand, cars × routed commute (r=0.92, 0.87 in era 1). Everyone takes the shortest route even when full — there is no rerouting, and that is the honest limit of a distance field |

...
| **V6** | **Lots with something on them** - fences, hedges, paths, wider colour range, more silhouettes | L | E0 | **Done 2026-09-06.** Six silhouettes per category (four new, two clones separated), fourteen house roofs and six flat ones, and residential boxes set back so there is a garden to put a hedge and a path in. `VARIANTS` was written down twice and is now written down once — raising one copy and not the other makes every building of the new variants vanish silently. `client_smoke` hashes every variant's vertices and fails on a duplicate |
...
| **E3** | **Ribbons** - carriageway, kerbs, sidewalks, junction boxes, connector curves; marking canvas per chunk | M | E2, V4 | **Done 2026-09-06.** `surfaceAt` returns a `y`: carriageway at `heightAt + 0.02`, pavement a 0.15 m kerb above it. 9 chunks live holding 76,294 triangles, build p95 7 ms against 8 ms, budget gate green on all 32 rows. Markings are dashed ribbons rather than a canvas (Q31); the low-pitch camera now orbits the ground rather than `y = 0` |
| **E4** | **Street camera and collision** - walk controls, `CollisionWorld` (walls from lots, patches from sidewalks), enter/exit, touch | M | E3, V5 | **Done 2026-09-06.** `walkthrough` walks 161 km over every carriageway and both pavements of a saturated 96×96 — 0 unfinished, 0 refusals, 0 cliffs — and walks head-on into all 1,127 lots without entering one; `passability` clean at 26.00 m narrowest. `play_smoke` enters and leaves by key and by zoom on both viewports and both projections. The walker is pure and lives in `client/life/`, so all of it is tested in node |
| **E5** | **L3 facades** - the generated spec, the four category grammars, roofs with eaves, reveals built outward, signage canvases, emissive buckets | L | E2, D5 | **Done 2026-09-06.** 25.7k triangles a chunk, 2 meshes a group, build p95 6 ms; a facade is 700–1,100 triangles. `reports/style-sheet-street.png` shoots all three styles from the pavement. L2 and L3 take their colour from one `familyColour`, so they cannot disagree. The tier budgets moved to 320k/140k/40k — 200k was set before L3 existed and nine chunks of facade is 200k on its own |
| **E6** | **Time of day** - preset list per rig, clock-driven with an off switch, lit windows, lamp pools, following snapped shadow frustum | M | E5 | **Done 2026-09-06.** Three presets that SCALE the rig rather than replace it, so all three styles keep their character; a night frame is 266,538 of 320,000 triangles with 8 lamps lit of 269 held. `a11y_smoke` measures the overlay bands at both hours (122 apart by day, 41 at night, floor 30). `ui_smoke` drives all four settings values through the panel. The prop pass had been building nothing since E5 — `corridorsIn` was called without its junction argument — and the night rig is what found it |
...
| **P2** | **Ink and grade** - depth-texture target, second-difference ink, split-tone grade, FXAA, desktop tier only, governor-gated | M | P1, V2 | **Done 2026-09-06.** A second difference of linearised depth, so a wall of windows draws no lines and the roofline draws one; two passes rather than three (ink and grade fold together); the line is three texels over a 1.5x supersample, because at one it averages away in the downsample. The grade follows the hour. And the budget had been measuring the full-screen quad — two triangles — for as long as any post pass has existed, so the ladder never stepped down for `pixel` |

...
| **V2** | **Quality tiers** with a frame-time governor | S | - | As planned; `budget_gate` at three tiers |
| **E1** | **Lane graph** in the model, node-tested | S | E0 | Every link has a successor; signals phased |
| **V1** | **Traffic you can see** - local car-following on the lane graph, density from `tiles.traffic`, instanced, capped, a ladder rung | M | E1, V2, D6 | Cars bunch behind a signal on a saturated road; no hash moves; `budget_gate` green |
| **V3** | **Ground that is not a checkerboard** - corner colour blend on natural tiles only, mottle, distance-to-street tone | S | E0 | Screenshot at three zooms; built land stays flat |
| **V4** | **Real relief** - `RELIEF_M`, corridor flattening, seating on the lowest corner, height-field picking, every flat layer re-checked | M | E0, D7 | Overlay, zone tint, lawn, markings and ghost show no seam on a slope at max relief |
| **V5** | **Tilt camera** - perspective option, per-chunk `tilePixels`, frustum bounds, sky dome | M | V4, D7 | Snapped yaws still snap; LOD plan stable across a tilt sweep; `play_smoke` passes both projections |
